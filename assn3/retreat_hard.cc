...
std::set<std::pair<int, int>> edges, duplicates;
...
int main() {
  ...
  for (int t = 0; t < T; t++) {
    ...
    bridges.clear();
    collect_bridges(0);
    std::set<std::pair<std::pair<int, int>, int>> bridges_filtered;
    for (const auto &p : bridges) {
      if (duplicates.count(p.first))
        continue;
      bridges_filtered.insert(p);
    }
    if (bridges_filtered.empty()) {
      std::cout << "-1\n";
      continue;
    }
    std::cout << std::min_element(bridges_filtered.begin(),
                                  bridges_filtered.end(),
